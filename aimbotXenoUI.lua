...
function Aimbot:SetWallHack(enabled)
    ...
    print("WallHack: " .. (enabled and "Enabled" or "Disabled"))
    return true
end
...
local Window = Library:CreateWindow({Name = "Aimbot Menu", Description = "Aimbot Settings"})

local AimbotTab = Window:AddTab("Aimbot Settings")

-- 加载Aimbot按钮
AimbotTab:AddButton("加载 Aimbot", function()
    Aimbot:Init()
    Library:SendNotification("Aimbot", "Aimbot 已加载", 3)
end)

-- 停止Aimbot按钮
AimbotTab:AddButton("停止 Aimbot", function()
    Aimbot:Disable()
    Library:SendNotification("Aimbot", "Aimbot 已停止", 3)
end)

-- WallHack开关
AimbotTab:AddToggle("WallHack", function(Value)
    Aimbot:SetWallHack(Value)
    Library:SendNotification("WallHack", Value and "已开启" or "已关闭", 3)
end, false)

-- FOV大小滑块
AimbotTab:AddSlider("FOV 大小", function(Value)
    Aimbot:SetPOVConfig({FOV = Value})
end, 10, 200, 30)

-- 线条粗细滑块
AimbotTab:AddSlider("线条粗细", function(Value)
    Aimbot:SetPOVConfig({Thickness = Value})
end, 1, 15, 4)

-- 线段数量滑块
AimbotTab:AddSlider("线段数量", function(Value)
    Aimbot:SetPOVConfig({Segments = Value})
end, 12, 100, 36)

-- 重叠度滑块
AimbotTab:AddSlider("重叠度", function(Value)
    Aimbot:SetPOVConfig({Overlap = Value})
end, 0, 0.3, 0.05, 0.01)

-- 透明度滑块
AimbotTab:AddSlider("透明度", function(Value)
    Aimbot:SetPOVConfig({Transparency = Value})
end, 0, 1, 0.3, 0.1)

-- 颜色选择器
AimbotTab:AddColorPicker("FOV 颜色", function(Color)
    Aimbot:SetPOVConfig({Color = Color})
end, Color3.fromRGB(255, 0, 0))

-- 瞄准部位选择
local aimParts = {"Head", "HumanoidRootPart", "Torso"}
AimbotTab:AddDropdown("瞄准部位", function(Value)
    Aimbot.Settings.AimPart = Value
    Library:SendNotification("Aim Part", "瞄准部位: " .. Value, 3)
end, "Head", aimParts)

-- 团队检查开关
AimbotTab:AddToggle("团队检查", function(Value)
    Aimbot.Settings.TeamCheck = Value
    Library:SendNotification("Team Check", Value and "已开启" or "已关闭", 3)
end, false)

-- 墙壁检查开关
AimbotTab:AddToggle("墙壁检查", function(Value)
    Aimbot.Settings.WallCheck = Value
    Library:SendNotification("Wall Check", Value and "已开启" or "已关闭", 3)
end, true)

-- 最大距离滑块
AimbotTab:AddSlider("最大距离", function(Value)
